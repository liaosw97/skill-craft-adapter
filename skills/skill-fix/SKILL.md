...
- 示例: `/skill-fix --ref https://standard.md ./my-skill`
...
**用户参考优先原则**: 当用户参考覆盖某评估维度时，修复标准以用户参考为准。
...
- 问题现象信号（优先匹配）: "skill 有问题" "skill 输出不正确" "skill 修复后还是不行"
...
以下规则在整个会话期间持续有效，不因步骤推进或对话长度而衰减：
...
- **评估可并行**：问题扫描阶段可 dispatch 多个子 agent，各收到目标内容 + 检查清单原文（复制不转述），边界明确
- **修复不可并行**：修复间有依赖，必须串行执行
- **合并检查**：去重 + 一致性验证 + 计数校验（各子 agent 发现数之和 == 总问题数）
